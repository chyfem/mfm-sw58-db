-- MFM SW58 Araromi Akure CMS — PostgreSQL Schema for Render.com
-- Run once via Render Shell: psql $DATABASE_URL -f database/schema.sql
...
-- Auto-update trigger
CREATE OR REPLACE FUNCTION _set_updated_at() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = NOW(); RETURN NEW; END; $$ LANGUAGE plpgsql;
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_b') THEN CREATE TRIGGER trg_b BEFORE UPDATE ON branches     FOR EACH ROW EXECUTE FUNCTION _set_updated_at(); END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_u') THEN CREATE TRIGGER trg_u BEFORE UPDATE ON users        FOR EACH ROW EXECUTE FUNCTION _set_updated_at(); END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_m') THEN CREATE TRIGGER trg_m BEFORE UPDATE ON members      FOR EACH ROW EXECUTE FUNCTION _set_updated_at(); END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname='trg_t') THEN CREATE TRIGGER trg_t BEFORE UPDATE ON transactions  FOR EACH ROW EXECUTE FUNCTION _set_updated_at(); END IF;
END $$;
